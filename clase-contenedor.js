...
module.exports = class Contenedor  {
    ...
    async getById(numId){
        const knex = require("knex")(this.configKnex)
        try{
            const objBuscado = await knex.from(this.stringNombreTabla)
            .where("id", "=", numId)
            .then((data)=>{
                return data;
            })
...
//ej campoValor --> {precio:1700}
    async replaceById(numId, campoValor, valor){
        const knex = require("knex")(this.configKnex)
        try{
        const objReemplazar = await knex.from(this.stringNombreTabla)
        .where("id", "=", numId)
        .update(campoValor)
        .then(()=>{
   ...
   async getAll(){
    const knex = require("knex")(this.configKnex)
    try{
        const data = await knex.from(this.stringNombreTabla)
        .select("*")
        .then((res)=>{
    ...
    async deleteById(number){
        const knex = require("knex")(this.configKnex)
        try{
            const borrarObjeto = knex.from(this.stringNombreTabla)
            .where("id", "=", number)
            .del()
            .catch((e)=>{
                console.log(e)
    ...
    async deleteAll(){
        const knex = require("knex")(this.configKnex)
        try{
            const borrarTodo = await knex.from(this.stringNombreTabla)
            .del()
            .catch((e)=>{
                console.log(e)
            })
